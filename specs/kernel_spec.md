...
手順:
  1. x_scale = mean(|x|),  x_norm = x / x_scale    (absmean正規化)
  2. packed 1-bit重みを展開しながら accumulate:
     - bit=1 → +1, bit=0 → -1
     - x_norm の各要素に対して: bit=1 なら加算、bit=0 なら減算
  3. y = accumulator * w_scale * x_scale
```
...
手順:
  1. x_scale = mean(|x|),  x_norm = x / x_scale    (float32のまま)
  2. packed 1-bit重みを展開しながら accumulate:
     - bit=1 → +1, bit=0 → -1
     - x_norm[i] に対して: bit=1 なら加算、bit=0 なら減算
     (BitConv1d と同じ方式。x を符号ビットに変換してはならない)
  3. y = accumulator * w_scale * x_scale + bias
```

**注意:** x は float32 のまま使う。sign(x) への変換(XNOR+popcount)は
入力の大きさ情報を破壊し、出力がほぼ固定値(≈bias)になる。
