...
class AudioDegrader:
    ...
    def sample_params(self) -> DegradationParams:
        ...
        # SNR for noise-based maskers (not competing speakers — they use SIR)
        if params.masker_type in ("stationary", "modulated", "babble_multi"):
            params.snr = random.uniform(*cfg.snr_range)

        # SIR for competing speakers (separate from SNR-based maskers)
        ...
        # Speech rate
        if random.random() < cfg.speed_change_prob:
