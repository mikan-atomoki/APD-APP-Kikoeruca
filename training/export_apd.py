...
def serialize_bitconv1d(layer_buf: io.BytesIO, weight_buf: WeightBuffer,
    ...
    w_scale = (float(conv.weight.detach().abs().mean())
               * conv._fan_in_rsqrt
               * float(conv.scale.data.item()))
...
def serialize_bitlinear(layer_buf: io.BytesIO, weight_buf: WeightBuffer,
    ...
    w_scale = (float(linear.weight.detach().abs().mean())
               * linear._fan_in_rsqrt
               * float(linear.scale.data.item()))
