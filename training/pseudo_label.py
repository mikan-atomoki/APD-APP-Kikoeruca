...
def compute_apd_label(
    ...
    if params.masker_type == "competing_1_2" and params.sir is not None:
        # SIR-dependent penalty for competing speakers
        # SIR=-5dB → ~0.30, SIR=5dB → ~0.55, SIR=20dB → ~0.85
        sir_factor = 1.0 / (1.0 + np.exp(-0.2 * (params.sir - 5.0)))
        masker_penalty = 0.3 + 0.55 * sir_factor
    else:
